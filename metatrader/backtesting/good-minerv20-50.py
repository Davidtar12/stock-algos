...
# -----------------------------
data = bfs._get_data_for_bt(timeframe, symbol, 100)
...
class MinerviniVCP(Strategy):
    ...
    def next(self):
        ...
        ma20 = self.data.SMA_20[-1]
        ma50 = self.data.SMA_50[-1]

        if np.isnan(ma20) or np.isnan(ma50):
            return

        uptrend = close > ma20 and ma20 > ma50 and close > ma50

        print(f"Close={close:.2f} | MA20={ma20:.2f} | MA50={ma50:.2f} | Uptrend={uptrend}")
